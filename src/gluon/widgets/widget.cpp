  ...
  if (!tokens.is_empty()) {
    ...
    switch (first_token.token_type) {
      case TokenType::kString: {
        ASSERT(tokens.element_count() == 1,
               "A Color as a string can only be defined by one token");
        first_token.text.erase(
            std::remove_if(first_token.text.begin(), first_token.text.end(),
        ...
        switch (hash) {
          case kRgbHash: {
            if (values.element_count() == 3) {
              color = color::FromRGBA((i32)values[0], (i32)values[1],
                                      (i32)values[2]);
            } else {
              LOG_F(ERROR,
          ...
          case kHslHash: {
            if (values.element_count() == 3) {
              color = color::FromHSLA((i32)values[0], (i32)values[1],
                                      (i32)values[2]);
            } else {
              LOG_F(ERROR,
          ...
          default: {
            if (first_token.text.compare("Color") == 0) {
              ASSERT(tokens.element_count() == 3 &&
                         tokens[2].token_type == TokenType::kIdentifier,
                     "We are looking for `Color.ColorName`");
              if (auto it = color::kColorsByName.find(tokens[2].text);
                  it != color::kColorsByName.end()) {
                color = it->second;
              } else {
                LOG_F(ERROR,
                      ...
                      first_token.line, tokens[2].text.c_str());
              }
            } else {
              LOG_F(ERROR,
                    "Line %d: Could not parse Color given '%s' "
                    "value",
                    first_token.line, first_token.text.c_str());
            }
          } break;
        }
      }
...
void BuildDependencyGraph(Widget* root_widget, Widget* current_widget) {
  for (auto id : current_widget->dependency_ids) {
    Widget* dep = id == "Parent" ? current_widget->parent
                                 : GetWidgetById(root_widget, id);
...
void BuildExpressionEvaluators(Widget* root_widget, Widget* current_widget) {
  ...
  for (auto expr : current_widget->geometry_expressions) {
    ...
    auto expression = shunting_yard::Expression::Build(expr.second, root_widget,
                                                       current_widget);

    current_widget->evaluators.add(
