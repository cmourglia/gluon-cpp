...
void Image::ParserPropertyInternal(parser::Node::Ptr node,
                                   const u32 node_hash) {
  switch (node_hash) {
    case static_cast<u32>(NodeHash::kUrl): {
      ...
      if (FilePath.extension().string() == ".svg") {
        ...
        image_size = {image_info.raster_image->image->w,
                      image_info.raster_image->image->h};
      }
    } break;
...
void Image::PostEvaluate() {
  if (image_info.is_vectorial) {
    ...
    image_info.svg_image =
        nsvgParseFromFile(image_url.c_str(), "px", beard::min(size.x, size.y));
  } else {
#if 0
